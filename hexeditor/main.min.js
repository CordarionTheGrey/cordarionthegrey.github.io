...
return node;};newTileNode=function(pos,id,color){var children,x$,xy;children=[newSVG('polygon',{points:"9.5,-5.5 9.5,5.5 0,11 -9.5,5.5 -9.5,-5.5 0,-11",fill:color}),newSVG('text',{'class':'tile-label'}),newSVG('text',{'class':'tile-sublabel',y:6})];if(id!=null){children.push(newSVG('text',{'class':'tile-id',y:-6}));}
...
return x$;};placeTile=function(pos,id,color){var node,polygon,label;console.assert(!(pos in model.tiles),"A tile is placed above another one");node=newTileNode(pos,id,color);model.svgRoot.appendChild(node);polygon=node.firstChild;label=polygon.nextSibling;return model.tiles[pos]={pos:pos,color:color,id:id,node:node,polygon:polygon,label:label,sublabel:label.nextSibling,pic:null};};unplaceTile=function(pos){var tile;tile=model.tiles[pos];if(!tile){console.assert(false,"Cannot unplace non-existent tile");return;}
...
return model.edges[getEdgeKey(pos0,pos1)];};placeEdge=function(pos0,pos1,thickness){var t,key,a,b,o,x1,y1,x2,y2,line;if(pos0>pos1){t=pos0;pos0=pos1;pos1=t;}
key=getEdgeKey(pos0,pos1);console.assert(!(key in model.edges),"An edge is placed above another one");a=model.tiles[pos0];b=model.tiles[pos1];if(a.id<b.id){o=a;a=b;b=o;}
switch(Vec.sub(b.pos,a.pos)){case 0x0001:x1=9.5;y1=-5.5;x2=9.5;y2=5.5;break;case 0x00FF:x1=-9.5;y1=-5.5;x2=-9.5;y2=5.5;break;case 0x0100:x1=9.5;y1=5.5;x2=0;y2=11;break;case 0xFF00:x1=-9.5;y1=-5.5;x2=0;y2=-11;break;case 0xFF01:x1=9.5;y1=-5.5;x2=0;y2=-11;break;case 0x01FF:x1=-9.5;y1=5.5;x2=0;y2=11;}
line=newSVG('line',{x1:x1,y1:y1,x2:x2,y2:y2,style:"stroke-width:"+thickness});a.node.appendChild(line);return model.edges[key]={pos0:pos0,pos1:pos1,thickness:thickness,line:line};};main=function(){var svg,coeff,rootX,rootY,i$,ref$,len$,palId,pal,placeButton,unplaceButton;removeEventListener('DOMContentLoaded',main);model.svgRoot=$id('svg-root');svg=model.svgRoot.parentNode;coeff=0.5/DEFAULT_SCALE;rootX=svg.clientWidth*coeff;rootY=svg.clientHeight*coeff;moveTileRaw(model.svgRoot,rootX,rootY);for(i$=0,len$=(ref$=PALETTES).length;i$<len$;++i$){palId=ref$[i$];pal=$id(palId);(fn$.call(this,palId,pal.getElementsByClassName('contents')[0]));}
...
x$=model.auxTile=placeTile(pos,null,'#F2F2F2');x$.node.classList.add('ghost');}});svg.addEventListener('mousedown',function(ev){var pos,tile,ref$,that,ref1$,input,edge;if(ev.button){return;}
if(model.placerAction==='place'){if(model.auxTile){unplaceTile(model.auxTile.pos);placeTile(model.auxTile.pos,++model.idCounter,'#EEEEEE');model.auxTile=null;}}else if(model.placerAction==='unplace'){pos=Vec.fromCartesian(ev.offsetX-rootX,ev.offsetY-rootY,11);if(pos in model.tiles){unplaceTile(pos);}}else if(model.activePal==='palette-tile-inspector'){pos=Vec.fromCartesian(ev.offsetX-rootX,ev.offsetY-rootY,11);if((tile=model.tiles[pos])&&tile!==model.curInspected){if(model.curInspected){model.curInspected.node.classList.remove('inspected');}
model.curInspected=tile;tile.node.classList.add('inspected');$id('inspected-id').textContent='#'+tile.id;$id('inspected-color').value=tile.color;$id('inspected-label').value=tile.label.textContent;$id('inspected-sublabel').value=tile.sublabel.textContent;}}else if(model.activePal==='palette-edge-inspector'){pos=Vec.fromCartesian(ev.offsetX-rootX,ev.offsetY-rootY,11);if(tile=model.tiles[pos]){if(model.curInspected1&&!model.curInspected2&&Vec.dist(model.curInspected1.pos,tile.pos)<=1){if(tile!==model.curInspected1){model.curInspected2=tile;}}else{model.curInspected1=tile;model.curInspected2=null;}
$id('inspected-edge').textContent=((that=((ref$=model.curInspected1)!=null?ref$.id:void 8))!=null?'#'+that:'?')+" — "+((that=((ref1$=model.curInspected2)!=null?ref1$.id:void 8))!=null?'#'+that:'?');input=$id('inspected-edge-thickness');if(!(input.disabled=!model.curInspected2)){edge=getEdge(model.curInspected1.pos,model.curInspected2.pos);if(edge){model.curInspectedEdge=edge;}else{edge=model.curInspectedEdge=placeEdge(model.curInspected1.pos,model.curInspected2.pos,DEFAULT_EDGE_THICKNESS);}
...
if(that=model.auxTile){unplaceTile(that.pos);model.auxTile=null;}});$id('inspected-color').addEventListener('input',function(){var that;if(that=model.curInspected){that.color=that.polygon.style.fill=this.value;}});$id('inspected-label').addEventListener('input',function(){var that;if(that=model.curInspected){that.label.textContent=this.value;}});$id('inspected-sublabel').addEventListener('input',function(){var that;if(that=model.curInspected){that.sublabel.textContent=this.value;}});addCustomListener('palette-tile-inspector','close',function(){if(model.curInspected){model.curInspected.node.classList.remove('inspected');model.curInspected=null;}
$id('inspected-id').textContent="<Не выбрано>";$id('inspected-color').textContent=$id('inspected-label').textContent=$id('inspected-sublabel').textContent="";});$id('inspected-edge-thickness').addEventListener('input',function(){model.curInspectedEdge.thickness=model.curInspectedEdge.line.style.strokeWidth=this.value;});addCustomListener('palette-edge-inspector','close',function(){model.curInspected1=model.curInspected2=null;$id('inspected-edge').textContent="? — ?";$id('inspected-edge-thickness').value="";});function fn$(palId,contents){contents.classList.add('hidden');pal.getElementsByClassName('header')[0].addEventListener('click',function(){var that;if(that=model.activePal){$id(that).getElementsByClassName('contents')[0].classList.add('hidden');fireCustomListener(that,'close');if(palId===that){model.activePal="";return;}}
