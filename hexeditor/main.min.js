...
line=createNode('line',{x1:x1,y1:y1,x2:x2,y2:y2,style:"stroke-width:"+thickness});view.edges[getEdgeKey(pos0,pos1)]=line;view.tiles[pos0].appendChild(line);};updateEdgeInView=function(view,edge){view.edges[getEdgeKey(edge.pos0,edge.pos1)].style.strokeWidth=edge.thickness;};removeEdgeFromView=function(view,edge){var x$,ref$,key$,ref1$;x$=(ref1$=(ref$=view.edges)[key$=getEdgeKey(edge.pos0,edge.pos1)],delete ref$[key$],ref1$);x$.parentNode.removeChild(x$);};formatTileId=function(model,pos){if(pos!=null){return'#'+model.tiles[pos].id;}else{return'?';}};loadModel=function(){var m,m0,e;m=m0=localStorage.currentMigration||"";if(m<'2019-02-26'){m='2019-02-26';delete localStorage.map;}
if(m!==m0){localStorage.currentMigration=m;}
try{return deserializeModel(localStorage.map);}catch(e$){e=e$;console.error(e);return createModel();}};saveModel=function(model){localStorage.map=serializeModel(model);};main=function(){var model,view,i$,x$,ref$,len$,y$,uiModel,svg,coeff,rootX,rootY,palId,pal,placeButton,unplaceButton;removeEventListener('DOMContentLoaded',main);model=loadModel();view=createView();for(i$=0,len$=(ref$=Object.values(model.tiles)).length;i$<len$;++i$){x$=ref$[i$];addTileToView(view,x$);}
...
placeButton=$id('place-tile');unplaceButton=$id('unplace-tile');placeButton.addEventListener('click',function(){unplaceButton.classList.remove('pressed-button');placeButton.classList.toggle('pressed-button');uiModel.placerAction=uiModel.placerAction!=='place'?'place':"";});unplaceButton.addEventListener('click',function(){placeButton.classList.remove('pressed-button');unplaceButton.classList.toggle('pressed-button');uiModel.placerAction=uiModel.placerAction!=='unplace'?'unplace':"";});$id('reset-map').addEventListener('click',function(){var x$,that;if(!confirm("Вы действительно хотите полностью очистить карту?")){return;}
model=createModel();saveModel(model);x$=view.svgRoot;while(that=x$.firstChild){x$.removeChild(that);}
view=createView();});addCustomListener('palette-tile-placer','close',function(){placeButton.classList.remove('pressed-button');unplaceButton.classList.remove('pressed-button');uiModel.placerAction="";});svg.addEventListener('mousemove',function(ev){var pos,that;if(uiModel.placerAction!=='place'){return;}
...
if(pos in model.tiles){uiModel.auxTile=null;}else{addTileToView(view,uiModel.auxTile=createTile(pos,null,'#F2F2F2')).classList.add('ghost');}});svg.addEventListener('mousedown',function(ev){var that,pos,tile,input,edge;if(ev.button){return;}
...
if((that=uiModel.auxTile)!=null){removeTileFromView(view,that);uiModel.auxTile=null;}});$id('inspected-color').addEventListener('input',function(){var that,x$;if((that=uiModel.curInspected)!=null){x$=model.tiles[that];x$.color=this.value;updateTileInView(view,x$);saveModel(model);}});$id('inspected-label').addEventListener('input',function(){var that,x$;if((that=uiModel.curInspected)!=null){x$=model.tiles[that];x$.label=this.value;updateTileInView(view,x$);saveModel(model);}});$id('inspected-sublabel').addEventListener('input',function(){var that,x$;if((that=uiModel.curInspected)!=null){x$=model.tiles[that];x$.sublabel=this.value;updateTileInView(view,x$);saveModel(model);}});addCustomListener('palette-tile-inspector','close',function(){var that;if((that=uiModel.curInspected)!=null){view.tiles[that].classList.remove('inspected');uiModel.curInspected=null;}
$id('inspected-id').textContent="<Не выбрано>";$id('inspected-color').textContent=$id('inspected-label').textContent=$id('inspected-sublabel').textContent="";});$id('inspected-edge-thickness').addEventListener('input',function(){var that;if(that=uiModel.curInspectedEdge){that.thickness=this.value;updateEdgeInView(view,that);saveModel(model);}});addCustomListener('palette-edge-inspector','close',function(){uiModel.curInspected1=uiModel.curInspected2=null;$id('inspected-edge').textContent="? — ?";$id('inspected-edge-thickness').value="";});function fn$(palId,contents){contents.classList.add('hidden');pal.getElementsByClassName('header')[0].addEventListener('click',function(){var that;if(that=uiModel.activePal){$id(that).getElementsByClassName('contents')[0].classList.add('hidden');fireCustomListener(that,'close');if(palId===that){uiModel.activePal="";return;}}
