...
try{return deserializeModel(localStorage.map);}catch(e$){e=e$;console.error(e);return createModel();}};saveModel=function(model){localStorage.map=serializeModel(model);};main=function(){var model,view,i$,x$,ref$,len$,y$,uiModel,svg,coeff,rootX,rootY,palId,pal,placeButton,unplaceButton,resetMap;removeEventListener('DOMContentLoaded',main);model=loadModel();view=createView();for(i$=0,len$=(ref$=Object.values(model.tiles)).length;i$<len$;++i$){x$=ref$[i$];addTileToView(view,x$);}
